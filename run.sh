...
#
# Usage:
#   ./fix-all-the-things.sh [--project-dir <path>] <issue_number> [issue_number2] ...
#   ./fix-all-the-things.sh 5960
#   ./fix-all-the-things.sh 5960 5961 5962
#   ./fix-all-the-things.sh --project-dir ~/custom/path/strato-platform 5960
#
# Requirements:
#   Tools:
...
#                  Can be set via .env file in this directory or exported
#
#   Project Directory:
#     - Defaults to ../strato-platform relative to this script
...
Options:
  --project-dir <path>  Path to strato-platform repository (default: ../strato-platform)
  --help, -h            Show this help message

Environment:
  GH_TOKEN              GitHub personal access token (required)
                        Can be set in .env file in this directory

Examples:
  $0 5960                                        # Fix issue 5960
  $0 5960 5961 5962                              # Fix multiple issues
  $0 --project-dir ~/code/strato-platform 5960   # Custom project path
EOF
            exit 0
            ;;
        *)
...
# Fall back to hardcoded defaults if no issues provided
...
if [ -z "${GH_TOKEN:-}" ]; then
    ...
    echo "Please create a .env file with: GH_TOKEN=your_github_token"
    echo "Or export it: export GH_TOKEN='your-github-token'"
    echo "See .env.sample for template"
    exit 1
fi
...
if ! command -v jq &> /dev/null; then
    echo "ERROR: jq not found. Install with: apt-get install jq (Ubuntu/Debian) or brew install jq (macOS)"
    exit 1
fi

if ! command -v timeout &> /dev/null; then
    echo "ERROR: timeout command not found. Install coreutils from your package manager"
    exit 1
fi
...
if [ -z "$PROJECT_DIR" ]; then
    ...
    echo "Default location ../strato-platform does not exist"
    echo "Please specify with: --project-dir /path/to/strato-platform"
    exit 1
fi

if [ ! -d "$PROJECT_DIR" ]; then
    echo "ERROR: Project directory does not exist: ${PROJECT_DIR}"
    echo "Please specify a valid path with: --project-dir /path/to/strato-platform"
    exit 1
fi
...
if [ "$REPO_NAME" != "strato-platform" ]; then
    echo "ERROR: Project directory must be named 'strato-platform'"
...
if ! git rev-parse --is-inside-work-tree &> /dev/null; then
    echo "ERROR: Not inside a git repository: ${PROJECT_DIR}"
    exit 1
fi
...
log_success "Found strato-platform at ${PROJECT_DIR}"
